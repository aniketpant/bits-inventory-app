...
@forelse($users as $user)
    ...
    <a class="btn btn-inverse" rel="get_info" href="<?php echo url('common/alloted_locations'); ?>" data-id="{{ $user->details->id }}">{{ $user->user_name }}</a>
    @elseif(Session::get('primary_uri') == 'admin/dashboard/controls/manage_alloted_inventory')
    <a class="btn btn-inverse" rel="get_info" href="<?php echo url('common/user_roles'); ?>" data-id="{{ $user->details->id }}">{{ $user->user_name }}</a>
    @endif
@empty
